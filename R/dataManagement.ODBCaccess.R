...
dt.fishStatData <- getNewestVersion("dt.fishStatData", fileloc("iData")) # created in dataPrepFishStat.R; only needed for composites
dt.USDAnutrients <- getNewestVersion("dt.USDAnutrients", fileloc("iData")) # created in dataPrepUSDANuts.R ; contains nutrient info for USDA codes used
dt.cookinRetn <- getNewestVersion("dt.cookinRetn", fileloc("iData")) # IMPACT_code specific cooking retention values and conversion rates from FAOSTAT to 'retail'

# lists needed everywhere -----
composites <- sort(unique(dt.compositesLU$composite))
...
# dt.IMPACTcodeLU is only default values for single commodities.
dt.singleCodeLU <- data.table::as.data.table(openxlsx::read.xlsx("data-raw/NutrientData/nutrientDetails/IMPACTCodeLookup.xlsx"))


test <- merge(combined.wide, dt.compositesLU, by = "item_code")
test[, c("FAOSTAT_code", "usda_desc", "item_name") := NULL]

# add fish composites to test


test <- merge(test, dt.USDAnutrients, by = "usda_code", all.x = TRUE)
# now manipulate the data

#' keep just info on the nutrients we're interested in. No longer needed. It's in dt.USDAnutrients
...
# #commented out cooking retention info setup -----
...
# get just the default codes for IMPACT codes that just have one USDA code; work on composites below -----
...
#' create dt to hold all the LU values - dt.nutrients; first put single crop value in
dt.nutrients <- inDT
...
for (i in 1:nrow(dt.compositesLU)) {
  ...
  fName <- temp[[1]]
  USDAcodes <- temp[[2]]
  nut_data <- NUT_DATA[usda_code %in% USDAcodes & Nutr_No %in% nutcodes,] # keep just the commodities for this composite
  #  nut_data <-  nut_data[Nutr_No %in% nutcodes, ] combined with previous line
  nutr_def <-  NUTR_DEF[Nutr_No %in% nutcodes, ]
  data.table::setnames(nutr_def, old = "Units", new = "unit")
  #weight <- WEIGHT[usda_code %in% USDAcodes,]
  food_des <- FOOD_DES[usda_code %in% USDAcodes,]
  ...
  dt <- merge(nutr_def, nut_data, by = "Nutr_No") #combine nutrient codes and names
  dt <- merge(dt, food_des, by = "usda_code") #combine nutrient info with food descriptive info
  dt.phytatecomp <- dt.phytateLU[usda_code %in% USDAcodes, ]
  dt.phytatecomp[, c("edible_share.fromPhytateSource") := NULL] # assume the USDA refuse values are correct
  dt.phytatecomp <- unique(dt.phytatecomp)
  dt <- merge(dt, dt.phytatecomp, by = c("usda_code"), all.x = TRUE) #add phytate info to commodities in the composite commodity
...
# prepare FAOSTAT crop production data -----
dt.prod.crops <- copy(dt.FAOSTAT.prod.crops)
...
data.table::setnames(dt.prod.crops, old = c("Country Code", "Item Code", "Item", "Element", "Unit"),
                     new = c("FAOSTAT_code", "item_code", "item", "element", "unit"))

dt.prod.crops <- dt.prod.crops[element %in% c("Production") & unit %in% "tonnes",]
...
dt.prod.crops.long[, Production := prod.sum][, c("FAOSTAT_code", "prod.sum", "element", "unit") := NULL]
dt.prod.crops.wide <- unique(dt.prod.crops.long)
dt.prod.crops.wide[, c("Production")][is.na(dt.prod.crops.wide[, c("Production")])] <- 0

# prepare FAOSTAT animal production data ------
dt.prod.animals <- copy(dt.FAOSTAT.prod.animals)

keepListCol <- c("Country Code", "Item Code", "Item", "Element", "Unit", keepListYears.composites)
dt.prod.animals <- dt.prod.animals[, (keepListCol), with = FALSE]
data.table::setnames(dt.prod.animals, old = c("Country Code", "Item Code", "Item", "Element", "Unit"),
                     new = c("FAOSTAT_code", "item_code", "item", "element", "unit"))
...
# prepare trade data ------
dt.trade <- copy(dt.FAOSTAT.trade)
keepListCol <- c("Country Code", "Item Code", "Item", "Element", "Unit", keepListYears.composites)
dt.trade <- dt.trade[, (keepListCol), with = FALSE]
data.table::setnames(dt.trade, old = c("Country Code", "Item Code", "Item", "Element", "Unit"),
                     new = c("FAOSTAT_code", "item_code", "item", "element", "unit"))
...
dt.fb[, foodAvail := mean(foodAvail), by = eval(data.table::key(dt.fb))] #the average of food availability of keepListYears.composites
...
for (i in cropComposites) {
  fName <- paste0("dt.comp_", i) # note that this is the same as the file created above called dt.xxx where xxx is i
  dt.temp.fName <- getNewestVersion(fName) # nutrient composition
  ...
  dt.LU <- dt.compositesLU[composite %in% i & remove %in% 0, ]
...
# work on fish -----
#' fish composite data prepared in dataPrepFishStat.R
fishStatData <- getNewestVersion("fishStatData", fileloc("iData"))
...
# # work on food group stuff to add to dt.nutrients. Now done in dataPrepUSDANuts.R (as of Mar 4, 2018)
...
# all the kcals stuff has been moved (hopefullly) to dataPrepUSDANuts.R as of Mar 4, 2018
# add kcals.ethanol_g column
...
temp <- names(inDT)
Encoding(temp) <- "unkown"
setnames(inDT, old = names(inDT), new = temp)
