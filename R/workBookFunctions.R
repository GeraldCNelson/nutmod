# Intro -------------------------------------------------------------------
#This script contains functions to generate openxlsx::spreadsheets of the nutrition requirements results
...
f.write.incShare.sheet <- function(incShare, wb) {
  ...
  #wb is the openxlsx::spreadsheet file set up in workSheetCreation.R
  ...
  incShare.wide <-
    openxlsx::spread(incShare[, c("region", "year", "Pcon")], year, Pcon)
...
f.write.nut.sheet <- function(nutdf, wb) {
  ...
  print(shtName)
  nutdf.wide <-
    openxlsx::spread(nutdf[, c("region", "year", "value")], year, value)
  openxlsx::addWorksheet(wb, sheetName = shtName)
  openxlsx::writeData(
    wb,
    nutdf.wide,
